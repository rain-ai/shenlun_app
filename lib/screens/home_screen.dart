...
import 'custom_question_review_screen.dart';
...
────────────────────
该软件只用于免费学习交流，切勿牟利''', style: const TextStyle(fontSize: 13, height: 1.7)),
            ),
            actions: [
              TextButton(
                ...
                : SingleChildScrollView(
                  ...
                  child: Column(
                    ...
                    children: [
                      ...
                      const SizedBox(height: 12),
                      _buildCustomQuestionReview(theme),
                      const SizedBox(height: 12),
        ...
        child: InkWell(
          onTap:
              () => Navigator.push(
                context,
                MaterialPageRoute(
                  builder: (_) => const SummaryPracticeScreen(),
                ),
              ),
          borderRadius: BorderRadius.circular(16),
          child: Padding(
  ...
  Widget _buildCustomQuestionReview(ThemeData theme) {
    return Container(
      padding: const EdgeInsets.all(16),
      decoration: BoxDecoration(
        color: Theme.of(context).colorScheme.surface,
        ...
        child: InkWell(
          onTap:
              () => Navigator.push(
                context,
                MaterialPageRoute(
                  builder: (_) => const CustomQuestionReviewScreen(),
                ),
              ),
          borderRadius: BorderRadius.circular(16),
          child: Padding(
    ...
    final items = [
      {
        ...
        'count': _typeStats['概括归纳'] ?? 0,
      },
      {
        'icon': Icons.psychology_rounded,
        'label': '综合分析',
        'color': const Color(0xFFA29BFE),
        'count': _typeStats['综合分析'] ?? 0,
      },
      {
        'icon': Icons.lightbulb_rounded,
        'label': '提出对策',
        'color': const Color(0xFFF9CA24),
        'count': _typeStats['提出对策'] ?? 0,
      },
      {
                    ...
                    onTap:
                        () => Navigator.push(
                          ...
                          MaterialPageRoute(
                            builder:
                                (_) => QuestionListScreen(
                                  questionType: item['label'] as String,
                                ),
                          ),
                        ),
                    borderRadius: BorderRadius.circular(14),
  ...
  Widget _buildTypeList(ThemeData theme) {
    ...
    return Container(
      ...
      child: Column(
        children:
            _typeStats.entries.where((e) => colors.containsKey(e.key)).map((e) {
  ...
  Future<List<String>> _analyzeWeakness() async {
    ...
    final sorted =
        weakness.entries.toList()..sort((a, b) => a.value.compareTo(b.value));
    final result = <String>[];
    ...
    return Container(
      ...
      decoration: BoxDecoration(
        gradient: LinearGradient(
          colors: [
            const Color(0xFFF9CA24).withOpacity(0.08),
            const Color(0xFFE94560).withOpacity(0.08),
          ],
        ),
        borderRadius: BorderRadius.circular(16),
      ...
      child: Row(
        children: [
          Container(
            ...
            height: 48,
            decoration: BoxDecoration(
              color: const Color(0xFFF9CA24).withOpacity(0.12),
              borderRadius: BorderRadius.circular(14),
            ),
            child: const Icon(
          ...
          SizedBox(
            ...
            child: ElevatedButton.icon(
              onPressed:
                  () =>
                      _checkAPIAndNav(const CommentaryExerciseScreen(), '简评学习'),
      ...
      child: Row(
        children: [
          ...
          SizedBox(
            ...
            child: ElevatedButton.icon(
              onPressed:
                  () => _checkAPIAndNav(const SummaryExerciseScreen(), '概括与分析'),
              icon: const Icon(Icons.play_arrow, size: 16),
              label: const Text('开始', style: TextStyle(fontSize: 13)),
              style: ElevatedButton.styleFrom(
    ...
    if (qs.isEmpty) {
      if (mounted)
        ScaffoldMessenger.of(
          context,
        ).showSnackBar(const SnackBar(content: Text('题库暂无匹配题目')));
      return;
    }
    if (!mounted) return;
    Navigator.push(
    ...
    if (key.isEmpty) {
      if (mounted)
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text('「$label」需要 DeepSeek API Key，请在「我的」页面设置'),
  ...
  void _startWeaknessPractice() async {
    ...
    if (weaknesses.isEmpty) {
      if (mounted)
        ScaffoldMessenger.of(
          context,
        ).showSnackBar(const SnackBar(content: Text('练习数据不足，请先完成至少5道题')));
      return;
    }
    final weakTypes =
        weaknesses
    ...
    Navigator.push(
      context,
      MaterialPageRoute(
        builder: (_) => WeaknessPracticeScreen(weakTypes: weakTypes),
      ),
    );
  }
}
